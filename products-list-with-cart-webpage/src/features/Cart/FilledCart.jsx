...
const FilledCart = () => {
  ...
  return (
    <>
      {cart.map((prod) => (
        <ProductInCart prod={prod} key={prod.id} />
      ))}

      <p className="cart--price-line">
        <span>Order Total</span>
        <span className="cart--price">{formatePrice(cartTotalPrice)}</span>
      </p>
      <div className="cart--delivery-note">
        <svg className="icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 21 20">
      ...
      <OrderBtn onClick={() => open()}>Confirm Order</OrderBtn>
      <Overlay>
        <OrderConfirmed />
      </Overlay>
    </>
